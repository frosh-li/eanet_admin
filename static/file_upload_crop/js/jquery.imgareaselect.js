...
jQuery.imgAreaSelect.init = function(img, options) {
    ...
    function viewX(x) {
        return parseInt(x) + parseInt(imgOfs.left) + parseInt(parScroll.left) - parseInt(parOfs.left)
    }
    function viewY(y) {
        return parseInt(y) + parseInt(imgOfs.top) + parseInt(parScroll.top) - parseInt(parOfs.top)
    }
    function selX(x) {
        return parseInt(x) - parseInt(imgOfs.left) - parseInt(parScroll.left) + parseInt(parOfs.left)
    }
    ...
    function evX(event) {
        return parseInt(event.pageX) + parseInt(parScroll.left) - parseInt(parOfs.left)
    }
    function evY(event) {
        return parseInt(event.pageY) + parseInt(parScroll.top) - parseInt(parOfs.top)
    }
    function adjust() {
    ...
    function update(resetKeyPress) {
        ...
        $outTop.css({
            left: (left + parseInt(selection.x1)) + 'px',
            top: top + 'px',
            width: selection.width + 'px',
            height: selection.y1 + 'px'
        });
        $outRight.css({
            left: (left + parseInt(selection.x2)) + 'px',
            top: top + 'px',
            width: (imgWidth - parseInt(selection.x2)) + 'px',
            height: imgHeight + 'px'
        });
        $outBottom.css({
            left: (left + parseInt(selection.x1)) + 'px',
            top: (top + parseInt(selection.y2)) + 'px',
            width: selection.width + 'px',
            height: (imgHeight - parseInt(selection.y2)) + 'px'
        });
    ...
    function areaMouseDown(event) {
        ...
        adjust();
        if (options.resizable == true && resize.length > 0) {
        ...
        } else jQuery(document).mousedown(event);
        return false
    }
...
console.log(selection);
    ...
    function doMove(newX1, newY1) {
        x2 = parseInt((x1 = newX1)) + parseInt(selection.width);
        y2 = parseInt((y1 = newY1)) + parseInt(selection.height);
    ...
    function startSelection(event) {
        ...
        if(options.resizable == true){
        	...
        	selection.x1 = 0;
        	selection.y1 = 0;
	        selection.x2 = selection.x1 + parseInt(options.width);
    	    selection.y2 = selection.y1 + parseInt(options.height);
			selection.width = options.width-1;
        	selection.height = options.height-1;
    	}
        ...
        if(options.resizable == true){
        	doResize(selection.x2, selection.y2);
        }
    ...
    function cancelSelection() {
        jQuery(document).unbind('mousemove', startSelection).mousemove(selectingMouseMove);
        $a.add($o).hide()
    }
    function imgMouseDown(event) {
        ...
        if(options.resizable == true)
        	jQuery(document).one('mouseup', cancelSelection);
        return false
    }
