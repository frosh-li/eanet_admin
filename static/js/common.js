...
function click_nav_menu(obj_node) {
	...
	if (url.length > 0 && (url != "#")) {
		...
		//var p_offset = $("a[node='"+node+"']").parents('li[type="node"]').find("a:first").offset().top;
		var top = $(".sidebar").scrollTop();
		
		set_cookie("offset_p",top);
	} else {
...
$(document).ready(function() {
	$(".sidebar .nav a, .node_ul li a[node]").click(function(e) {
		click_nav_menu($(this));
	})
	...
	breadcrumb="";
	var top_height = $("#navbar").outerHeight(true);
	var  offset_p = get_cookie("offset_p");
	$(".sidebar").scrollTop(offset_p-top_height);
